...
> 自动抓取[雪球网](https://xueqiu.com)推荐/热门板块与指定话题的帖子和评论，从中提取有价值的线索，输出**可直接复制给大模型分析的提示词 md**。支持持续运行、SQLite 去重存储，可打包为独立 EXE。
...
- **价值线索提取（Layer 1）**：每 6 小时自动扫描全部评论，规则打分剔除灌水，按标的分组提取有价值线索（小道消息/产业链/业绩订单/多空方向），生成**含大模型提示词的 `clues_*.md`**
...
- **默认只产 md**：每轮只输出「整理内容 + 大模型提示词」的 `.md`，不落地 JSON；需要结构化数据时加 `--json`
...
└── data/               # 运行时自动生成（已 gitignore）
    ...
    │   ├── clues_YYYYMMDD_HHMMSS.md   # 价值线索（推荐/热门，按标的分组 + 大模型提示词）
    │   ├── insight_<标识>_YYYYMMDD_HHMMSS.md   # 价值线索（话题，含大模型提示词）
...
把「推荐/热门 板块」与「指定话题」两套抓取整合到**一个程序**，运行时用 `--mode` 区分，**默认两者都抓**。无论哪种模式，策略完全一致：抓到的评论都经过 Layer 1 规则打分、剔除灌水，输出**价值线索 md（含可直接复制给大模型分析的提示词）**。

> 默认**只产出 `.md`**，不落地任何 JSON。需要结构化数据时加 `--json`。
...
4. 生成产出：
   - `data/exports/insight_<标识>_<时间戳>.md` —— 按标的分组的候选评论 + **可直接复制给大模型的提示词区块**（默认产出）
   - `data/exports/insight_<标识>_<时间戳>.json` —— 结构化候选数据（需 `main(write_json=True)` 或 `--json`）

> 提示词区块已内置【话题背景】与任务指令（价值评估 / 去重合并 / 按重要性与可信度排序 / 生成小道消息日报）。**输出要求明确为「直接输出 markdown 日报正文，不要返回 JSON」**，复制后粘贴到任意大模型即可获得可读的分析报告。
...
- `clues_<时间戳>.md` —— 按标的分组的候选评论 + **可直接复制给大模型**的提示词区块（默认产出）
...
阈值 `≥5` 进入候选池；同时 `is_meaningless()` 会剔除灌水（顶/沙发/666/纯表情/纯重复字符等）。
...
> 价值线索的成品文件（默认只有含提示词的 `.md`）由 `clue_extractor.generate_clue_files()` **统一产出**，三个入口（`scraper.py`、`insight_extractor.py`、`xueqiu.py`）都走同一函数，保证格式与口径完全一致；传 `write_json=True`（或统一入口 `--json`）才会额外写 JSON。
